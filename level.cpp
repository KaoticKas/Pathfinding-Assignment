...
void cLevel::Draw(void)
{
	...
	for (int i = 0; i < GRIDHEIGHT; i++)
	{
		for (int j = 0; j < GRIDWIDTH; j++)
		{
			...
			if (map[i][j] == 0)
			{
				if ((gAStar.completed) && (gAStar.inPath[i][j] == true)) SDL_RenderCopy(gMainRenderer, tileRouteTexture, NULL, &position);
				else if ((gAStar.completed) && (gAStar.closed[i][j] == true)) SDL_RenderCopy(gMainRenderer, tileClosedTexture, NULL, &position);
